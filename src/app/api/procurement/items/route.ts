...
export async function GET(req: Request) {
  ...
  const rows = await query<Record<string, unknown>>(
    `SELECT i.*, c.name AS category_name
     ...
     ORDER BY i.sort_order ASC, i.group_name ASC NULLS LAST, i.name ASC`,
...
export async function POST(req: Request) {
  ...
  const rows = await query<Record<string, unknown>>(
    `INSERT INTO procurement_items
      (category_id, group_name, name, need_qty, have_qty, in_transit_qty, notes, link, sort_order)
    ...
    [
      categoryId,
      body.groupName ? String(body.groupName).trim() : null,
      name,
      Math.max(0, Number(body.needQty ?? 0)),
      Math.max(0, Number(body.haveQty ?? 0)),
      Math.max(0, Number(body.inTransitQty ?? 0)),
